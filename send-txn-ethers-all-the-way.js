...
async function sendTxn() {
  ...
  const addressTo = "0xE2a18e03e7fE8dc974614BA6De081cd782239424"; // first account from ganache
  ...
  const gasLimit = await provider.estimateGas({
    from: address2, // address of the MPC wallet derived from the public key generated from the BD SDK. See lines 78-93
    to: addressTo,
    value: 0,
    data
  });
...
const unsignedTransaction = {
  ...
  const { ecSignature, r, s,  } = ASN1ParseSecp256k1Signature(
    aggregatedSignatureDER
  );
  console.log("R2", r);
  console.log("S2", s);
  ...
  // );
  // // rawTxn.v = "0x" + recoveryID; // Error received: Legacy txs need either v = 27/28 or v >= 37 (EIP-155 replay protection), got v = 0
  // rawTxn.r = "0x" + r;
  // rawTxn.s = "0x" + s;
  ...
  const sig = {
    ...
    const recoveredAddress = await utils.recoverAddress(unsignedTxHash, sig); // TODO: Figure out why this address does not match with address2. Possibly related to v,r,s values
