...
const { bytesToHex, toBytes } = require("@ethereumjs/util");
...
async function sendTxn() {
  ...
  let unsignedTx1 = transaction.getHashedMessageToSign();
  // unsignedTx1 = rlp.encode(unsignedTx1);
  // unsignedTx1 = Buffer.from(unsignedTx1).toString('hex')
  console.log("hash1 generated: ", unsignedTx1);

  const rsTx = await ethers.utils.resolveProperties(rawTxn);
  const raw = ethers.utils.serializeTransaction(rsTx); // returns RLP encoded tx
  let unsignedTx2 = ethers.utils.keccak256(raw); // as specified by ECDSA
  console.log("hash2 generated: ", unsignedTx2);

  // The Txn hash to sign
  let msg = JSON.stringify(rawTxn);
  let sha256 = crypto.createHash("sha256");
  sha256.update(msg);
  let unsignedTx3 = sha256.digest();
  console.log("hash3 generated: ", unsignedTx3);
  ...
  // let r1 = aggregatedSignatureDER.slice(0, 66);
  // let s1 = "0x" + aggregatedSignatureDER.slice(66, 130);
  // let v1 = "0x" + aggregatedSignatureDER.slice(130, 132);
  ...
  const recoveredAddress = await ethers.utils.recoverAddress(unsignedTx1, sig);
