...
const { providers, utils, Transaction } = require("ethers");
...
async function sendTxn() {
  ...
  // const gasLimit = (await provider.getBlockNumber("")).gasLimit;
  ...
  const rsTx = await utils.resolveProperties(rawTxn);
  const raw = utils.serializeTransaction(rsTx); // returns RLP encoded tx
  // console.log("raw", raw);
  let unsignedTx2 = utils.keccak256(raw); // as specified by ECDSA
  // unsignedTx2 = hexToString(msgHash2);
...
// sign = splitSignature(sign);
  ...
  const recoveredAddress = await utils.recoverAddress(unsignedTx1, sig);
