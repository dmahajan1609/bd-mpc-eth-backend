...
async function sendTxn() {
  ...
  const tranxn = LegacyTransaction.fromTxData(rawTxn, common);
  // this returns the keccak256 hash of the RLP encoded message
  unsignedTxHash = tranxn.getHashedMessageToSign()

  console.log("tranxn", tranxn)
  console.log("hash3 generated: ", unsignedTxHash);
  ...
  let signedTxn = LegacyTransaction.fromTxData(rawTxn, common);
  ...
  web3.eth.sendSignedTransaction(bytesToHex(signedTx.serialize()), function(error, hash) {
    if (!error) {
